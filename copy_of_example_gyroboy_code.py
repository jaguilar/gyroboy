    ...
    while True:
        ...
        if new_action is not None:
            ...
            if new_action.steering != 0:
                action = Action(
                    drive_speed=action.drive_speed, steering=new_action.steering
                )
            else:
                action = new_action
            yield action
        ...
        if ultrasonic_sensor.distance() < 250:
            ...
            yield Action(drive_speed=BACKWARD_SLOW.drive_speed, steering=turn.steering)
            action_timer.reset()
            while action_timer.time() < 4000:
                yield
...
while True:
    ...
    while True:
        ...
        if control_loop_count == 0:
            ...
            average_control_loop_period = (
                control_loop_timer.time() / 1000 / control_loop_count
            )
